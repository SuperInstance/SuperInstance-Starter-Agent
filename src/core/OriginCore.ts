...
export class OriginCore implements IOriginCore {
   ...
   */
  private async callTeacher(task: Task): Promise<any> {
   ...
   */
  private fallbackProcess(task: Task): any {
    return {
      type: 'fallback',
  ...
  private getEquipmentUsage(equipment: Equipment): number {
    ...
    for (const entry of recentHistory) {
      if (entry.type === 'output' && entry.data.equipmentUsed?.includes(equipment.slot)) {
        usage++;
      }
    }
   ...
   */
  private addHistoryEntry(type: HistoryEntry['type'], data: any): void {
   ...
   */
  getState(): { id: string; equipment: string[]; confidence: number; rates: RateBasedUpdate } {
    return {
      id: this.id,
      equipment: Array.from(this.equipment.keys()),
      confidence: this.history.entries.slice(-5).reduce((sum, e) => sum + e.confidence, 0) / 5,
      rates: this.rates,
    };
  }
